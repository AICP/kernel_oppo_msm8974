...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
//gyx add for build error
static int generic_file_fsync(struct file *filp, struct dentry *dentry, int datasync)
{
	int res;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
//static int exfat_file_fsync(struct file *filp, int datasync) gyx modify for build
...

{
	...
	res = generic_file_fsync(filp,dentry,datasync);
	err = FsSyncVol(sb, 1);
